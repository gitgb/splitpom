...
<xsl:stylesheet
	...
	<xsl:template match="/p:project/p:name"> <!-- change name, add description -->
	...
	<xsl:template match="/p:project/p:properties"><xsl:message>Matched properties, adding wonder.version</xsl:message>
		<xsl:copy>		
	...
	<xsl:template match="/p:project/p:properties[not(p:skip.apple.frameworks)]">
		<xsl:message>No skip.apple.frameworks, will add the property</xsl:message>
		<xsl:copy><skip.apple.frameworks>false</skip.apple.frameworks>		 
	...
	<xsl:template match="p:modules"> <!-- snip --> </xsl:template>
	<xsl:template match="p:build/p:testSourceDirectory"> <!-- snip --> </xsl:template>
